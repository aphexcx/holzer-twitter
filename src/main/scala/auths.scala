...
object PropertiesClient {
  def apply(props: Properties): OAuthClient = {
    val consumer = new ConsumerKey(props getProperty "twitter.consumerKey",
      props getProperty "twitter.consumerKeySecret")
    val token = new RequestToken(props getProperty "twitter.accessToken",
      props getProperty "twitter.accessTokenSecret")
